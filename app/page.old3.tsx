...
export default function Home() {
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-[#FAFAF7] text-[#1A3A5C] scroll-smooth">
      <div className="fixed inset-0 bg-[radial-gradient(circle_at_top,_rgba(212,165,116,0.10),_transparent_44%)]" />

      <nav className="fixed left-0 top-0 z-30 flex w-full items-center justify-between border-b border-[#1A3A5C]/5 bg-[#FAFAF7]/85 px-6 py-4 backdrop-blur-md md:px-8">
        <button onClick={() => scrollTo("home")} className="text-xs font-bold tracking-[0.35em] text-[#1A3A5C] md:text-sm">
          ADAMIANI.AI
        </button>

        <div className="hidden items-center gap-7 text-sm font-medium text-[#5A6B7B] lg:flex">
          {[
            ...
            <button key={item.href} onClick={() => scrollTo(item.href)} className="transition hover:text-[#1A3A5C]">
              {item.label}
            </button>
          ))}
        </div>

        <div className="flex rounded-full border border-[#1A3A5C]/15 bg-white p-1 text-xs md:text-sm">
          {(["ka", "en", "ru"] as Lang[]).map((lng) => (
            <button
              key={lng}
              onClick={() => setLanguage(lng)}
              className={`rounded-full px-3 py-2 font-semibold transition md:px-4 ${
                language === lng ? "bg-[#1A3A5C] text-white" : "text-[#5A6B7B]"
              }`}
            >
      ...
      <section id="home" className="relative z-10 flex min-h-screen items-center justify-center px-6 pb-20 pt-32">
        ...
        >
          <div className="text-center lg:text-left">
            ...
            <h1 className="text-4xl font-bold leading-[1.08] text-[#1A3A5C] md:text-6xl lg:text-7xl">{t.title}</h1>
            <p className="mt-7 max-w-2xl text-lg leading-8 text-[#5A6B7B]">{t.description}</p>
            ...
            <div className="mt-12 grid gap-4 md:grid-cols-3">
              {[
                ...
                <div key={label} className="rounded-3xl border border-[#1A3A5C]/10 bg-white p-5 text-left shadow-sm">
          ...
          <motion.div
            ...
            className="rounded-[2rem] border border-[#1A3A5C]/10 bg-white p-6 shadow-xl"
          >
            <div className="mb-6 flex items-center justify-between border-b border-[#1A3A5C]/10 pb-5">
              <div>
                <p className="text-sm font-semibold text-[#1A3A5C]">Adamiani Care Corridor</p>
                <p className="text-xs text-[#8A97A3]">Case package preview</p>
              </div>
            ...
            <div className="space-y-4">
              {[
                ...
                <div key={title} className="flex gap-4 rounded-3xl bg-[#FAFAF7] p-4">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-sm leading-6 text-[#5A6B7B]">{text}</p>
                  </div>
                </div>
              ))}
            </div>
      ...
      <section id="journey" className="relative z-10 px-6 pb-28">
        <div className="mx-auto max-w-6xl">
          <h2 className="max-w-4xl text-3xl font-bold leading-snug text-[#1A3A5C] md:text-5xl">{t.journeyTitle}</h2>
          <div className="mt-12 grid gap-5 md:grid-cols-4">
            {t.journeySteps.map((step, index) => (
              <div key={step.title} className="rounded-[2rem] border border-[#1A3A5C]/10 bg-white p-7 shadow-sm transition hover:shadow-md">
                <div className="mb-7 flex h-12 w-12 items-center justify-center rounded-2xl bg-[#1A3A5C] text-lg font-bold text-white">
                  {index + 1}
                </div>
                <h3 className="text-xl font-semibold text-[#1A3A5C]">{step.title}</h3>
      ...
      <section className="relative z-10 px-6 pb-28">
        <div className="mx-auto max-w-6xl rounded-[2.5rem] border border-[#1A3A5C]/10 bg-white p-10 shadow-sm md:p-14">
          <p className="mb-4 text-sm font-semibold tracking-[0.3em] text-[#B8893F]">{t.problemTag.toUpperCase()}</p>
          <h2 className="max-w-4xl text-3xl font-bold leading-snug text-[#1A3A5C] md:text-5xl">{t.problemTitle}</h2>
          <p className="mt-6 max-w-4xl text-lg leading-8 text-[#5A6B7B]">{t.problemText}</p>
          <div className="mt-10 grid gap-4 md:grid-cols-4">
            {t.problemItems.map((item) => (
              <div key={item} className="rounded-3xl bg-[#FAFAF7] p-5 font-semibold text-[#1A3A5C]">
                {item}
              </div>
            ))}
          </div>
      ...
      <section id="passport" className="relative z-10 px-6 pb-28">
        <div className="mx-auto max-w-6xl">
          <p className="mb-4 text-sm font-semibold tracking-[0.3em] text-[#B8893F]">{t.passportTag}</p>
          <h2 className="text-4xl font-bold text-[#1A3A5C] md:text-6xl">{t.passportTitle}</h2>
          <p className="mt-5 max-w-3xl text-lg leading-8 text-[#5A6B7B]">{t.passportText}</p>
          <div className="mt-14 space-y-5">
            {t.passportItems.map((item) => (
              <div key={item.title} className="flex flex-col gap-4 rounded-[2rem] border border-[#1A3A5C]/10 bg-white p-6 shadow-sm transition hover:shadow-md md:flex-row md:items-center">
                <div className="rounded-2xl bg-[#1A3A5C] px-5 py-3 text-sm font-bold text-white">{item.year}</div>
                <div>
                  <h3 className="text-xl font-semibold text-[#1A3A5C]">{item.title}</h3>
                  <p className="mt-2 leading-7 text-[#5A6B7B]">{item.text}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <section id="clinics" className="relative z-10 px-6 pb-28">
        <div className="mx-auto grid max-w-6xl gap-10 lg:grid-cols-[0.95fr_1.05fr]">
          <div>
            <p className="mb-4 text-sm font-semibold tracking-[0.3em] text-[#B8893F]">{t.clinicTag.toUpperCase()}</p>
            <h2 className="text-4xl font-bold leading-tight text-[#1A3A5C] md:text-5xl">{t.clinicTitle}</h2>
            <p className="mt-6 text-lg leading-8 text-[#5A6B7B]">{t.clinicText}</p>
            <button onClick={() => scrollTo("waitlist")} className="mt-8 rounded-full bg-[#1A3A5C] px-8 py-4 font-semibold text-white transition hover:bg-[#15314E]">
              {t.primaryCta}
            </button>
          </div>
          <div className="grid gap-5">
            {t.clinicCards.map((card) => (
              <div key={card.title} className="rounded-[2rem] border border-[#1A3A5C]/10 bg-white p-7 shadow-sm">
                <div className="mb-4 flex h-11 w-11 items-center justify-center rounded-2xl bg-[#D4A574]/15 text-[#B8893F]">
                  <ClipboardCheck className="h-5 w-5" />
                </div>
                <h3 className="text-2xl font-semibold text-[#1A3A5C]">{card.title}</h3>
                <p className="mt-3 leading-7 text-[#5A6B7B]">{card.text}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section id="assistant" className="relative z-10 px-6 pb-28">
        <div className="mx-auto max-w-5xl rounded-[2.5rem] border border-[#1A3A5C]/10 bg-white p-8 shadow-xl md:p-10">
          <div className="mb-8 text-center">
            <p className="mb-4 text-sm font-semibold tracking-[0.3em] text-[#B8893F]">{t.assistantTag}</p>
            <h2 className="text-3xl font-bold text-[#1A3A5C] md:text-5xl">{t.assistantTitle}</h2>
            <p className="mx-auto mt-5 max-w-2xl text-lg leading-8 text-[#5A6B7B]">{t.assistantText}</p>
          </div>
          ...
          <div className="rounded-[2rem] border border-[#1A3A5C]/10 bg-[#FAFAF7] p-5">
            <div className="mb-5 flex items-center justify-between border-b border-[#1A3A5C]/10 pb-4">
              <div>
                ...
                <p className="mt-1 text-xs font-semibold text-[#B8893F]">{t.freeQuestionsLabel}: {freeQuestionsLeft}</p>
              </div>
              <div className="h-3 w-3 rounded-full bg-[#D4A574]" />
            </div>
            ...
            <div className="mt-5 flex flex-col gap-3 sm:flex-row">
              ...
              />
              <button type="button" onClick={sendMessage} className="h-14 rounded-full bg-[#1A3A5C] px-8 font-semibold text-white transition hover:bg-[#15314E]">
                {t.sendButton}
              </button>
            </div>
          </div>
      ...
      <section id="trust" className="relative z-10 px-6 pb-28">
        <div className="mx-auto max-w-6xl rounded-[2.5rem] border border-[#1A3A5C]/10 bg-white p-10 shadow-sm md:p-14">
          <p className="mb-4 text-sm font-semibold tracking-[0.3em] text-[#B8893F]">{t.trustTag.toUpperCase()}</p>
          <h2 className="max-w-4xl text-4xl font-bold text-[#1A3A5C] md:text-6xl">{t.trustTitle}</h2>
          <div className="mt-12 grid gap-5 md:grid-cols-4">
            {t.trustItems.map((item, index) => {
              const icons = [ShieldCheck, LockKeyhole, Stethoscope, HeartPulse];
              const Icon = icons[index] || ShieldCheck;
              return (
                <div key={item.title} className="rounded-[2rem] border border-[#1A3A5C]/10 bg-[#FAFAF7] p-7">
                  <div className="mb-5 flex h-11 w-11 items-center justify-center rounded-2xl bg-[#1A3A5C]/5 text-[#1A3A5C]">
                    <Icon className="h-5 w-5" />
                  </div>
                  <h3 className="text-xl font-semibold text-[#1A3A5C]">{item.title}</h3>
      ...
      <section id="waitlist" className="relative z-10 px-6 pb-32">
        <div className="mx-auto max-w-4xl rounded-[2.5rem] bg-[#1A3A5C] p-10 text-center md:p-14">
          <p className="mb-4 text-sm font-semibold tracking-[0.3em] text-[#D4A574]">{t.accessTag.toUpperCase()}</p>
          ...
          <div className="mx-auto mt-10 flex max-w-2xl flex-col gap-4">
            <input
              ...
              placeholder={t.name}
              className="h-16 rounded-full border border-white/20 bg-white/10 px-8 text-lg text-white outline-none placeholder:text-white/50 focus:border-[#D4A574]"
            />
            <input
              type="email"
              value={email}
            ...
            <button onClick={saveEmail} className="h-14 rounded-full bg-[#D4A574] px-8 font-semibold text-white transition hover:bg-[#C4955F]">
              {t.join}
            </button>
          </div>
        </div>
      ...
      {showRegisterModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-[#1A3A5C]/40 backdrop-blur-sm" onClick={() => setShowRegisterModal(false)} />
          <div className="relative z-10 w-full max-w-md rounded-[2rem] bg-white p-8 shadow-2xl md:p-10">
            <button type="button" onClick={() => setShowRegisterModal(false)} className="absolute right-5 top-5 text-[#8A97A3] transition hover:text-[#1A3A5C]">
              <X className="h-5 w-5" />
            </button>
            ...
            <h3 className="mt-3 text-2xl font-bold text-[#1A3A5C]">{t.modalTitle}</h3>
            <p className="mt-3 text-sm leading-6 text-[#5A6B7B]">{t.modalText}</p>
            <div className="mt-6 flex flex-col gap-3">
              <input
                ...
                onChange={(e) => setName(e.target.value)}
                placeholder={t.name}
                className="rounded-full border border-[#1A3A5C]/15 bg-[#FAFAF7] px-6 py-3.5 text-[#1A3A5C] outline-none placeholder:text-[#8A97A3] focus:border-[#D4A574]"
              />
              <input
                type="email"
                value={email}
              ...
              <button type="button" onClick={saveEmail} className="mt-2 rounded-full bg-[#D4A574] px-8 py-3.5 font-semibold text-white transition hover:bg-[#C4955F]">
                {t.modalButton}
              </button>
            </div>
          </div>
