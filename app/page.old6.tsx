...
const content = {
  ka: {
    nav: ["როგორ მუშაობს", "მკურნალობის მოთხოვნა", "Health Passport", "კლინიკები", "ნდობა"],
    ...
    trPreferred: "სად გსურთ მკურნალობა? (თურქეთი, გერმანია, UAE, სხვა)",
    trTimeline: "როდის გეგმავთ მკურნალობას?",
    trTimelineOpts: ["ამ თვეში", "1–3 თვეში", "მოგვიანებით"],
    trBudget: "სავარაუდო ბიუჯეტი (არასავალდებულო)",
    trMessage: "დამატებითი კომენტარი (არასავალდებულო)",
    trSubmit: "გამომიგზავნეთ ვარიანტები",
    trSuccess: "მოთხოვნა მიღებულია. Adamiani.ai მალე დაგიკავშირდებათ.",
    trFillRequired: "გთხოვთ შეავსოთ სახელი, ტელეფონი და მკურნალობის ტიპი.",
  },
  en: {
    nav: ["How it works", "Treatment Request", "Health Passport", "Clinics", "Trust"],
    heroBadge: "CROSS-BORDER HEALTHCARE",
    ...
    trCountryRes: "Which country do you live in?",
    trTreatment: "What treatment are you interested in?",
    trPreferred: "Where would you like treatment? (Turkey, Germany, UAE, other)",
    trTimeline: "When are you planning treatment?",
    trTimelineOpts: ["This month", "In 1–3 months", "Later"],
    trBudget: "Estimated budget (optional)",
    trMessage: "Additional comment (optional)",
    trSubmit: "Send Me Options",
    trSuccess: "Your request has been received. Adamiani.ai will contact you soon.",
    trFillRequired: "Please fill in name, phone and treatment type.",
  },
  ru: {
    nav: ["Как работает", "Запрос на лечение", "Health Passport", "Клиники", "Доверие"],
    heroBadge: "CROSS-BORDER HEALTHCARE",
    ...
    trCountryRes: "В какой стране вы живёте?",
    trTreatment: "Какое лечение вас интересует?",
    trPreferred: "Где вы хотите лечиться? (Турция, Германия, ОАЭ, другое)",
    trTimeline: "Когда планируете лечение?",
    trTimelineOpts: ["В этом месяце", "Через 1–3 месяца", "Позже"],
    trBudget: "Примерный бюджет (необязательно)",
    trMessage: "Дополнительный комментарий (необязательно)",
    trSubmit: "Отправьте варианты",
    trSuccess: "Ваша заявка получена. Adamiani.ai скоро свяжется с вами.",
    trFillRequired: "Пожалуйста, укажите имя, телефон и тип лечения.",
  },
} as const;
...
export default function Home() {
  ...
  const savePatientInquiry = async () => {
    if (!trName.trim() || !trPhone.trim() || !trTreatment.trim()) {
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-[#F5FAFF] text-[#062A4F] scroll-smooth">
      ...
      <nav className="fixed left-0 top-0 z-30 flex w-full items-center justify-between border-b border-[#DCEAF7] bg-[#F5FAFF]/85 px-6 py-4 backdrop-blur-md md:px-8">
        ...
        <div className="hidden items-center gap-7 text-sm font-medium text-[#50677F] lg:flex">
          {[
            ...
            { label: t.nav[3], href: "clinics" },
            { label: t.nav[4], href: "trust" },
          ].map((item) => (
      ...
      <section id="treatment-request" className="relative z-10 px-6 pb-28">
        <div className="mx-auto max-w-4xl rounded-[2.5rem] border border-[#DCEAF7] bg-gradient-to-b from-white to-[#F5FAFF] p-8 shadow-[0_28px_80px_rgba(6,42,79,0.14)] md:p-12">
          <div className="text-center">
            <p className="mb-4 text-sm font-semibold tracking-[0.3em] text-[#0EA5A4]">{t.trTag.toUpperCase()}</p>
            <h2 className="text-3xl font-bold text-[#062A4F] md:text-5xl">{t.trTitle}</h2>
            <p className="mx-auto mt-5 max-w-2xl text-lg leading-8 text-[#50677F]">{t.trSubtitle}</p>
          </div>

          {trSubmitted ? (
            <div className="mx-auto mt-10 max-w-xl rounded-[2rem] border border-[#0EA5A4]/30 bg-[#EAF5FF] p-10 text-center">
              <div className="mx-auto mb-5 flex h-14 w-14 items-center justify-center rounded-full bg-[#0EA5A4] text-white">
                <ShieldCheck className="h-7 w-7" />
              </div>
              <p className="text-lg font-semibold leading-8 text-[#062A4F]">{t.trSuccess}</p>
            </div>
          ) : (
            <div className="mt-10 grid gap-4">
              <div className="grid gap-4 md:grid-cols-2">
                <input
                  ...
                  placeholder={t.trName}
                  className="h-14 rounded-2xl border border-[#DCEAF7] bg-white px-5 text-[#062A4F] outline-none placeholder:text-[#7A8FA5] focus:border-[#0B63F6]"
                />
                <input
                  type="tel"
                  value={trPhone}
                  onChange={(e) => setTrPhone(e.target.value)}
                  placeholder={t.trPhone}
                  className="h-14 rounded-2xl border border-[#DCEAF7] bg-white px-5 text-[#062A4F] outline-none placeholder:text-[#7A8FA5] focus:border-[#0B63F6]"
                />
              </div>
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <select
                  ...
                  className={`h-14 rounded-2xl border border-[#DCEAF7] bg-white px-5 outline-none focus:border-[#0B63F6] ${trTimeline ? "text-[#062A4F]" : "text-[#7A8FA5]"}`}
                >
              ...
              <textarea
                ...
                placeholder={t.trMessage}
                rows={3}
                className="rounded-2xl border border-[#DCEAF7] bg-white px-5 py-4 text-[#062A4F] outline-none placeholder:text-[#7A8FA5] focus:border-[#0B63F6]"
              />

              <button
                type="button"
                onClick={savePatientInquiry}
                className="mt-2 h-14 rounded-full bg-[#0B63F6] px-8 font-semibold text-white transition hover:bg-[#084FC7]"
              >
                {t.trSubmit}
              </button>
            </div>
      ...
      <section id="clinics" className="relative z-10 px-6 pb-28">
        <div className="mx-auto grid max-w-6xl gap-10 lg:grid-cols-[0.95fr_1.05fr]">
          <div>
            ...
            <button onClick={() => scrollTo("treatment-request")} className="mt-8 rounded-full bg-[#0B63F6] px-8 py-4 font-semibold text-white transition hover:bg-[#084FC7]">
              {t.primaryCta}
            </button>
          </div>
